...
module.exports = class Shout extends Observer {
  ...
  constructor() {
    const regex = new RegExp(/^[^a-z]{4,}$|^who\ssaid\sthat$/g)
    super('shout', regex)
  }
  ...
  call(opts, respond) {
    if (opts.text === 'who said that') {
      if (this.lastShout) {
        const lastAt = moment(this.last.date_spoken).format('dddd, MMMM Do YYYY')
        return respond(`${this.last.nick} said that in ${this.last.chan} on ${lastAt}`)
      } else {
